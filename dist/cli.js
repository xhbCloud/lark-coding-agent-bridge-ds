...
function classifyHighRiskWorkingDirectory(real, requestedCwd, tempRealpath) {
...
function* translateEvent(raw) {
  ...
  // DEBUG: log unhandled events
  if (evt.type !== "result") return;
  // system (thinking_tokens etc) and other non-critical events: silently ignore
...
var ClaudeAdapter = class {
  ...
  run(opts) {
    ...
    // DEBUG: log the actual spawn args
    log.info("agent", "spawn-args", { binary: this.binary, args: args.join(' '), hasSession: Boolean(opts.sessionId), sessionId: opts.sessionId ? opts.sessionId.slice(0,20) : void 0 });
    ...
    child.stderr.on("data", (chunk) => {
      ...
      while (nl !== -1) {
        ...
        // DEBUG: log all stderr synchronously
...
async function sendManagedCard(channel, recipientId, card, opts = {}) {
  ...
  const { messageId } = await channel.send(
    recipientId,
    { cardId },
    opts.replyTo ? { replyTo: opts.replyTo } : void 0
  );
...
function renderCard(state, options = {}) {
  const _log = [];
  _log.push('TERMINAL:' + state.terminal + ' FOOTER:' + state.footer + ' BLOCKS:' + (state.blocks||[]).length);
  for (const b of (state.blocks||[])) {
    if (b.kind === 'text') _log.push('TEXT:' + b.content.slice(0,100));
    else if (b.kind === 'tool') _log.push('TOOL:' + (b.tool||{}).name);
  }
  _log.push('REASONING:' + ((state.reasoning||{}).content||'').slice(0,80));
  process.stderr.write('[RENDERCARD] ' + _log.join(' | ') + '\n');
  const elements = [];
  if (state.reasoning.content) {
    elements.push(reasoningPanel(state.reasoning.content, state.reasoning.active));
  }
  let _debugHasContent = false;
  for (const group of groupBlocks(state.blocks)) {
    if (group.kind === "text") {
      if (group.content.trim()) {
        _debugHasContent = true;
  ...
  if (state.terminal === "done" && !_debugHasContent && !state.reasoning?.content) {
    try { new Error('EMPTY_CARD_TRACE').stack.split('\n').slice(0,3).join('') } catch(e) {}
    throw new Error('[DEBUG_EMPTY] state=' + JSON.stringify({terminal:state.terminal,footer:state.footer,blocks:state.blocks.length,kinds:state.blocks.map(b=>b.kind+':'+(b.kind==='text'?b.content.slice(0,50):(b.tool||{}).name)).join(','),reasoning:(state.reasoning||{}).content?.slice(0,80)}));
  }
...
async function handleResume(args, ctx) {
  ...
  if (ctx.controls.profileConfig.agentKind === "codex") {
    ...
    if (history.length > 0 && identity2) {
      ...
      await ctx.channel.send(ctx.msg.chatId, { card: card3 }, { replyTo: ctx.msg.messageId });
      return;
    }
...
\u4F7F\u7528 \`/resume use ${nonce}\` \u6062\u590D\uFF0810 \u5206\u949F\u5185\u6709\u6548\uFF09\u3002`
    ...
    await ctx.channel.send(ctx.msg.chatId, { card: card2 }, { replyTo: ctx.msg.messageId });
    return;
  }
...
async function showResultCardInPlace(ctx, formMsgId, card) {
  try {
    ...
    await sendManagedCard(ctx.channel, ctx.msg.chatId, card).catch(
...
async function startRunFlow(input) {
  ...
  if (input.sessionCatalog) {
    ...
    if (catalogEntry?.agentId === "claude") {
      ...
      // DEBUG: validate resume session before running
      log.info("session", "catalog-resume", { sessionId, cwd: workspace.cwdRealpath });
  ...
  if (!resumeFrom && input.capability.agentId === "claude") {
    ...
    log.info("session", "resume-lookup", { scopeId: input.scopeId, cwd: workspace.cwdRealpath, found: Boolean(resumeFrom) });
    // 验证 resume session 是否真实有效，无效则清掉走全新会话
    if (sessionId) {
      // Quick validation: try --resume with a minimal check (exit code 0 = valid)
      try {
        const child = spawnProcess("D:/npm-global/node_modules/@anthropic-ai/claude-code/bin/claude.exe", ["--resume", sessionId, "--print", "ok", "--output-format", "text"], { stdio: ["ignore", "pipe", "pipe"] });
        const valid = await new Promise((resolve) => {
          let timedOut = false;
          const t = setTimeout(() => { timedOut = true; child.kill(); resolve(false); }, 10000);
          child.on("exit", (code) => { if (!timedOut) { clearTimeout(t); resolve(code === 0); } });
          child.on("error", () => { clearTimeout(t); resolve(false); });
        });
        if (!valid) {
          log.warn("session", "resume-invalid", { scopeId: input.scopeId, sessionId });
          resumeFrom = void 0;
          sessionId = void 0;
          input.sessions.clear(input.scopeId);
        }
      } catch (e) {
        log.warn("session", "resume-verify-error", { err: e.message });
        resumeFrom = void 0;
        sessionId = void 0;
      }
    }
...
async function runAgentBatch(deps) {
  ...
  // DEBUG: the actual sessionId that will be passed to adapter.run()
  log.info("session", "adapter-input", { scope, resumeFrom: flow.resumeFrom });
      ...
      const renderDone = processAgentStream(
        ...
        async (state) => {
          ...
          if (cardCtrl) {
            ...
            // render will pick up latestState when producer starts
          }
        }
      );
        ...
        chatId,
        {
          card: {
            ...
            producer: async (ctrl) => {
              ...
              cardCtrl = ctrl;
              // Ensure latestState is flushed immediately when producer starts
...
function createRuntimeAgent(profileConfig, appPaths2) {
  ...
  return new ClaudeAdapter({
    binary: process.env.LARK_CHANNEL_CLAUDE_BIN ?? "D:/npm-global/node_modules/@anthropic-ai/claude-code/bin/claude.exe",
    larkChannel
  });
}
